...
static StartScene* start = new StartScene();
static SceneLoader *loader = new SceneLoader(start);
static float FPS;
static int time_now, time_prev;
...
BOOL CreateGLWindow(char* title, int width, int height, int bits, bool fullscreenflag)
{
    ...
    loader->getCurrScene()->resizeGLScene(width, height);

	//if (!Scene->initGL())							// Initialize Our Newly Created GL Window
	if(!loader->getCurrScene()->initGL())
	{
		KillGLWindow();								// Reset The Display
							...
							LPARAM	lParam)			// Additional Message Information
{
    ...
    loader->getCurrScene()->winMsg(hWnd,uMsg,wParam,lParam);
	...
	switch (uMsg)									// Check For Windows Messages
	{
		...
		case WM_SIZE:								// Resize The OpenGL Window
		{
			...
			loader->getCurrScene()->resizeGLScene(LOWORD(lParam),HIWORD(lParam));
			return 0;								// Jump Back
		}
	}
					...
					int			nCmdShow)			// Window Show State
{
	...
	if (!CreateGLWindow("Coin Crash!",fullscreenWidth,fullscreenHeight,256,fullscreen))
	{
	...
	while(!done)									// Loop That Runs While done=FALSE
	{
		...
		else										// If There Are No Messages
		{
		    {
                //Scene->drawGLScene();
                loader->getCurrScene()->drawGLScene();
                SwapBuffers(hDC);
		    }
			...
			if((!active && !loader->getCurrScene()->drawGLScene()) || keys[VK_ESCAPE])
			{
